...
class HologramRenderer:
    ...
    def render(
        ...
        for i, j in self.inner_e:
            ...
            pygame.draw.aaline(surface, color, (p1[0], p1[1]), (p2[0], p2[1]))
    ...
    def _draw_hud_text(
        ...
        for fnt, text, color, pos in lines:
...
def draw_webcam_panel(
    ...
    if landmarks and hand_detected:
        ...
        for i, j in HAND_CONNECTIONS:
            ...
            x2 = ox + int(p2["x"] * dw)
            y2 = oy + int(p2["y"] * dh)
            pygame.draw.aaline(surface, COLORS.MAGENTA, (x1, y1), (x2, y2))
        for lm in landmarks:
            x = ox + int(lm["x"] * dw)
            y = oy + int(lm["y"] * dh)
...
class CyberdeckApp:
    ...
    def _draw_footer(self) -> None:
        ...
        for i, line in enumerate(self.logs[:5]):
            ...
            txt = self.font_sm.render(f"λ {line}", True, color)
